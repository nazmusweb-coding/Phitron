...
void mysort(Student *log, int totalStudents)
{
    for (int i = 0; i < totalStudents-1; i++)
    {
        ...
        {
            if (log[i].english_marks < log[j].english_marks)
            {
                myswap(log[i], log[j]);
            }
            else if (log[i].english_marks == log[j].english_marks)
            {
                if (log[i].math_marks < log[j].math_marks)
                {
                    myswap(log[i], log[j]);
                }
                else if (log[i].math_marks == log[j].math_marks)
                {
